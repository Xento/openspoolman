...
- Set `OPENSPOOLMAN_TEST_DATA=1` (or pass `--test-data` to the screenshot helper) to load a reproducible dummy dataset for the overview, tray fill, print history, and NFC flows via runtime monkeypatching (no alternate imports needed).
- Install the app dependencies (including Playwright) via `pip install -r requirements.txt`.
- Install the Chromium browser once via `python -m playwright install chromium`.
- Point print history at a local demo database by default (`data/demo.db`, auto-seeded with a few example jobs) or override with `OPENSPOOLMAN_PRINT_HISTORY_DB=/path/to/demo.db` or `--print-history-db` when using the screenshot helper.
...
- Run `python scripts/generate_screenshots.py` to start the app in test mode, open the configured routes in headless Chromium, and refresh the images under `docs/img/` (or another folder via `--output-dir`). Screenshots default to the viewport height unless a per-target/device `max_height` is set (or `full_page: true`), and `--max-height` provides a config-wide fallback.
- To generate pictures from real printer/Spoolman data, either point the generator at your running instance (`python scripts/generate_screenshots.py --mode live --base-url http://<host>:<port> --live-readonly`) or snapshot a live session once with `python scripts/export_live_snapshot.py --output data/live_snapshot.json` and then reuse it via `python scripts/generate_screenshots.py --test-data --snapshot data/live_snapshot.json`. You can still use environment variables (`OPENSPOOLMAN_TEST_DATA=1 OPENSPOOLMAN_TEST_SNAPSHOT=data/live_snapshot.json` or `OPENSPOOLMAN_LIVE_READONLY=1`) if you prefer. When live data is used or captured, the app and scripts automatically load `config.env` from the repository root (if present) so credentials and URLs are available even when running tools from subfolders. In live mode without an explicit `--base-url`, the generator now reuses `OPENSPOOLMAN_BASE_URL` from `config.env` if it is defined.
