...
- Run `python scripts/generate_screenshots.py` to start the app in test mode, open the key routes (`/`, `/fill`, `/print_history`, `/spool_info?spool_id=1`, `/write_tag`) in headless Chromium, and refresh the images under `docs/img/`.
- If long lists make a page too tall, pass `--max-height <pixels>` (for example, `--max-height 1200`) to crop each capture to the top portion of the view.
