...
- Configure which routes and devices get captured via `scripts/screenshot_config.json` (supports per-target or per-screenshot `max_height`, device-specific viewports, and automatic filename suffixing per device). Override or extend it with `--config <path>` and target specific devices with `--devices desktop,mobile` when you run the generator.
